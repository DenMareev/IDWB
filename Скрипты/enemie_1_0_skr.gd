...
func inflabelfunc():
	if Global.showinf:
		if randomint == 1:
			inflabel.text = "СЛАБЫЙ ЯД"
		elif randomint == 2:
			inflabel.text = "СИЛЬНЫЙ ЯД"
		elif randomint == 3:
			inflabel.text = "УКЛОНЕНИЕ"
		elif randomint == 4:
			inflabel.text = "+СИЛА"
	else:
		inflabel.text = " "
...
func atk1():
	await get_tree().create_timer(2).timeout
	GlobalSignals.emit_phurt("poison", 2 + int(might * 0.4))
	steprandomiser()
func atk2():
	await get_tree().create_timer(2).timeout
	GlobalSignals.emit_phurt("poison", 4 + int(might * 0.4))
	steprandomiser()
func def1():
	await get_tree().create_timer(2).timeout
	dodge += 1
